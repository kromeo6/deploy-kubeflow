...
resource "null_resource" "cert_manager_issuer" {
  provisioner "local-exec" {
    command = <<EOT
      while ! kustomize build manifests/common/cert-manager/kubeflow-issuer/base | kubectl apply --server-side --force-conflicts -f -; do echo "Retrying to apply cer manager issuer resources"; sleep 20; done
    EOT
  }

  provisioner "local-exec" {
    when    = destroy
    command = "kubectl delete -k manifests/common/cert-manager/kubeflow-issuer/base || true"
  }
